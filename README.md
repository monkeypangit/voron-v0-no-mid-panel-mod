...
## Z frame bracket
The trick to this mod is a printed bracket that connects the z extrusions to the back extrusion. It uses two M3x35 screws for rigidity instead in the same place as the blind joint screws in the stock version.
...
## Build information
It is a fairly invovled mod that requires replacing the power supply and heater from the stock BOM. It's probably best suited if you are building a self sourced v0 from scratch.
...
All the electronics are moved to the bottom bay. This requires using a compact toolboard, a Meanwell IRM-90 power supply and an AC bed heater with a compact Crydom SSR.

This mod is somewhat finicky to assemble. You need to thread the deck panel and the deck panel cover hover the z extrusions before you put the z brace in place. Here are some images go give you an idea of the parts involved.
...
The printed deck panel has tabs for mounting. The tabs allow for some flex to make sure the panel is rigidly mounted even if it is not exactly to size. It also has holes that allows the wires from the AB motors and toolhead to be passed through from the chamber to the electronics bay (see image below).

The deck panel is moved up slightly from the groove in the extrusions to sit flush with the top of the extrusions. You will likely loose a few millimiters of z-travel unless you are using a kirigami bed with 6mm build plate and shorter bed screws.

I use this mod for a v0 build with a 170x120mm (see image below). The slightly larger frame size makes it easier to fit the compnents into the electronics bay. The parts marked 170 in the stl folder is for a 170mm x build.
...
You will have to find mounting brackets for your choise of components by yourself. I used a Raspberry Pi 5 and a Fysetc R4 in my build. I used a bracket for the Pi and R4 and stuck everything down to the deck panel with VHB tape.
